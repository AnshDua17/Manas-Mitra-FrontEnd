...
const CounselorProfileCard = ({ counselor, onViewProfile }) => {
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm hover:shadow-md transition-all dark:bg-gray-800 dark:border-gray-700">
      <div className="flex items-start space-x-4 mb-4">
        <img src={counselor.photo} alt={counselor.name} className="w-20 h-20 rounded-full object-cover" />
        <div className="flex-1">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{counselor.name}</h3>
          <p className="text-sm text-gray-600 dark:text-gray-300 mb-2">{counselor.credentials}</p>
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {counselor.specializations.map(spec => <span key={spec} className="bg-blue-50 text-blue-700 text-xs px-2 py-1 rounded-full dark:bg-blue-900/50 dark:text-blue-400">{spec}</span>)}
      </div>
      <p className="text-sm text-gray-700 dark:text-gray-200 mb-4">{counselor.bio}</p>
      <div className="flex items-center justify-between pt-4 border-t border-gray-100 dark:border-gray-700">
        <div className="text-xs font-medium text-green-600 dark:text-green-500">{counselor.availability}</div>
...
const HowItWorksSection = () => {
  ...
  return (
    <div className="bg-blue-50 rounded-lg p-8 mb-8 dark:bg-gray-800">
        ...
        {faqs.map((faq, index) => (
          <div key={index} className="border-b border-gray-100 dark:border-gray-700 last:border-b-0">
            <button onClick={() => setOpenFAQ(openFAQ === index ? null : index)} className="flex items-center justify-between w-full py-4 text-left">
              ...
              {openFAQ === index ? <ChevronUp className="w-5 h-5 text-gray-500 dark:text-gray-400" /> : <ChevronDown className="w-5 h-5 text-gray-500 dark:text-gray-400" />}
            </button>
            {openFAQ === index && <div className="pb-4"><p className="text-gray-600 dark:text-gray-300">{faq.answer}</p></div>}
          </div>
        ))}
      </div>
    </div>
...
const CounselorConnectPage = () => {
  ...
  return (
    <div className="w-full">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-2">Counselor Connect</h1>
        <p className="text-lg text-gray-600 dark:text-gray-300">Schedule a confidential session with a certified on-campus professional.</p>
        <div className="flex items-center space-x-6 mt-4 text-sm text-gray-600 dark:text-gray-300">
          <div className="flex items-center"><Shield className="w-4 h-4 text-green-600 dark:text-green-500 mr-2" /><span>100% Confidential</span></div>
          <div className="flex items-center"><CheckCircle className="w-4 h-4 text-green-600 dark:text-green-500 mr-2" /><span>Licensed Professionals</span></div>
        </div>
      </div>
      ...
      <div className="my-12">
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Available Counselors</h2>
